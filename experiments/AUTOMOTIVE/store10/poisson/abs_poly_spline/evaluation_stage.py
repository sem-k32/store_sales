...
def polyInitializer(order: int) -> list[torch.Tensor]:
    NUM_MONTHS = 12
    return [
        torch.rand(order + 1, dtype=torch.float64)
...
if __name__ == "__main__":
    ...
    def train_callback(epoch: int, regressor: PoissonRegr, neg_ln_lk: float) -> None:
        with torch.no_grad():
            ...
            if epoch % 5 == 0:
                ...
                logger.log_image(f"prediction.png", fig)
    ...
    # vizualize validate approximation
    ...
    logger.log_image("train_prediction.png", fig)
